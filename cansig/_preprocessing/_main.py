...
from cansig._preprocessing._adatas import load_adatas, annotate_adata, pop_adatas, \
    DataRecorder, get_reference_groups
...
from cansig._preprocessing._utils import check_min_malignant_cells, \
    check_min_reference_cells, normalize, finalize_adata
from ..types import Scoring_dict, Pathlike


def preprocessing(input_adatas: List[Union[ad.AnnData, Pathlike]],
                  ...
                  min_genes: int = 700,
                  threshold_pct_mt_counts: float = 30.,
                  ...
                  figure_dir=None,
                  copy: bool = False,
                  min_malignant_cells: int = 50) -> ad.AnnData:
    if copy:
    ...
    for adata in pop_adatas(input_adatas, gene_list):
        annotate_adata(adata,
                       celltype_column,
                       malignant_celltypes,
                       undetermined_celltypes)

        adata = quality_control(adata, min_counts=min_counts,
                                max_counts=max_counts,
                                min_genes=min_genes,
                                threshold_mt=threshold_pct_mt_counts,
                                figure_dir=figure_dir)
        reference_cat = get_reference_groups(adata,
                                             celltype_column=celltype_column,
                                             reference_groups=reference_groups,
                                             min_reference_groups=min_reference_groups,
                                             min_reference_cells=min_reference_cells)

        if check_min_reference_cells(adata, reference_cat, min_reference_cells,
                                     min_reference_groups):
            normalize(adata)
            infercnv(adata, gene_order, window_size=window_size,
                     reference_cat=reference_cat, step=step, cnv_key=cnv_key)
            malignant_annotation(adata)

            if check_min_malignant_cells(adata, min_malignant_cells):
                subclonal_cluster(adata, cnv_key=cnv_key,
                                  batch_id_column=batch_id_column)
                recorder.append(adata)

    adata = recorder.concatenate()
    signature_scoring(adata, g2m_genes=g2m_genes, s_genes=s_genes,
