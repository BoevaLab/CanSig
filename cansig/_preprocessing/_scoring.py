...
def subset_gene_list(signature, signature_genes, gene_list):
    ...
    if len(signature_genes_subset) == 0:
        logger.warning(f"For signature {signature} the intersection between "
...
def signature_scoring(adata: ad.AnnData, g2m_genes: List[str], s_genes: List[str],
                      scoring_dict: Dict[str, list] = None):
    """
...
def score_malignant_cells(adata: ad.AnnData,
                          gene_list: List[str],
                          score_name: str,
                          n_bins: int = 25,
                          ctrl_size: int = 50):
    ...
    score = calculate_score(bdata, gene_list=gene_list, n_bins=n_bins,
                            ctrl_size=ctrl_size)
    adata.obs[score_name] = np.nan
    adata.obs.loc[bdata.obs_names, score_name] = score


def calculate_score(adata: ad.AnnData, gene_list: List[str], n_bins: int = 25,
                    ctrl_size: int = 50,
                    layer: str = _CONSTANTS.NORMALIZED) -> pd.Series:
    ...
    if issparse(adata.layers[layer]):
        obs_avg = pd.Series(
            np.array(
                _sparse_nanmean(adata.layers[layer], axis=0)).flatten(),
            index=adata.var_names,
        )  # average expression of genes
    else:
        obs_avg = pd.Series(
            np.nanmean(adata.layers[layer], axis=0),
            index=adata.var_names
        )  # average expression of genes
