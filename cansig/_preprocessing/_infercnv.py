...
def infercnv(adata: AnnData, gene_order: pd.DataFrame, reference_cat: List[str],
             window_size: int = 101, step: int = 10, cnv_key="cnv") -> None:
    """
    ...
    adata.var = adata.var.merge(
        gene_order, how="left", left_index=True, right_index=True
    )
    ...
    adata.var[_CONSTANTS.CNV_CALLED] = (
            adata.var[_GENE_ANNOTATION.CHROMOSOME].notnull()
            & ~adata.var[_GENE_ANNOTATION.CHROMOSOME].isin(_EXCLUDE_CHROMOSOME)
    )
    cnv_dict = {"chr_pos": chr_position, "window_size": window_size, "step": step}
    adata.uns[cnv_key], adata.obsm[f"X_{cnv_key}"] = cnv_dict, X_cnv


def get_gene_order(gene_order: Union[Pathlike, pd.DataFrame],
    ...
    if not isinstance(gene_order, pd.DataFrame):
        _DTYPE: Dict[str, str] = {_GENE_ANNOTATION.CHROMOSOME: "str",
...
def _validate_gene_order(gene_order: pd.DataFrame, gene_list: List[str]) -> None:
    ...
    Args:
        ...
        gene_list: List of genes present in all AnnDatas.
    """
    _EXPECTED_COLUMNS: Set[str] = {_GENE_ANNOTATION.CHROMOSOME, _GENE_ANNOTATION.START,
                                   _GENE_ANNOTATION.END}

    if (missing_columns := _EXPECTED_COLUMNS - set(gene_order.columns)) != set():
        raise KeyError(
            f"Columns {missing_columns} are missing from the gene annotation.")
    if gene_order.index.inferred_type != "string":
        raise ValueError(f"Expects gene names as index. Index of type {gene_order.index.dtype}"
                         f"found, instead of string.")
    ...
    if len(annotated_genes) / len(gene_list) < 0.5:
        _LOGGER.warning("Less then 50% of genes have their position annotated."
                        "Expect poor performance of InferCNV!", UserWarning)
