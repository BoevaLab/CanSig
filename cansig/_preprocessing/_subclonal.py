...
def subclonal_cluster(adata: AnnData, batch_id_column: str,
                      cnv_key: str = "cnv",
                      subclonal_key: str = _CONSTANTS.SUBCLONAL) -> None:
    """
    ...
    adata.obs.loc[bdata.obs_names, subclonal_key] = (
            bdata.obs[batch_id_column].astype(str) + "-" +
            bdata.obs[_CONSTANTS.CNV_LEIDEN].astype(str)
    )
