...
from typing import Union, List, Optional, Literal  # pytype: disable=not-supported-yet
import argparse
import os
import warnings
import logging
...
def get_cell_metamembership(cluster_memb: List, sig_index: List[List], clusters: np.ndarray) -> pd.DataFrame:
...
def run_metasignatures(
    ...
    if sim is None:
        _LOGGER.info("Computing the similarity between signatures.")
        sim = WRC.get_similarity_matrix(signatures=resdict["signatures"])
    ...
    gsea_metasig = {
        cl: pd.DataFrame(np.arange(len(meta_signatures[cl])), index=meta_signatures[cl], columns=["avg_rank"])
