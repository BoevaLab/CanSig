...
from cansig._preprocessing._scoring import (
    signature_scoring,
    update_scoring_dict,
...
def adata():
    ...
    adata.obs[_CONSTANTS.MALIGNANT] = [_CELL_STATUS.MALIGNANT] * 90 + [_CELL_STATUS.NON_MALIGNANT] * 10
    return adata


class TestScoring:
    def test_scoring(self, adata):
        signature_scoring(adata, g2m_genes=["gene_2"], s_genes=["gene_3"], scoring_dict={"Program_1": ["gene_0"]})
    ...
    def test_scoring_dict_none(self, adata):
        signature_scoring(adata, g2m_genes=["gene_2"], s_genes=["gene_3"], scoring_dict=None)
...
def test_subset_scoring_dict_none():
    assert update_scoring_dict(None, ["gene1", "gene2"]) is None


def test_update_cell_cycle_genes_none():
    g2m_genes, s_genes = update_cell_cycle_genes(None, None, list(set(_DEFAULT_G2M_GENES) | set(_DEFAULT_S_GENES)))
