...
from cansig._preprocessing._adatas import load_adatas, validate_adatas, \
    annotate_adata, _annotate_reference, _annotate_malignant, get_reference_groups, \
    DataRecorder
...
def adatas() -> List[ad.AnnData]:
    adatas = []
    gene_names = [[f"gene_{i}" for i in genes] for genes in
                  [range(4, 10), range(3, 8), range(5, 8)]]
...
class TestAnnotateAdata:
    def test_annotate_adata(self, adata):
        ...
        adata.obs[celltype_column] = ["evil"] * 4 + ["unknown"] * 2 + ["good"] * 2 + [
            "good_reference"] * 2
        ...
        expected_annotation = [_CELL_STATUS.MALIGNANT] * 4 + [_CELL_STATUS.UNDECIDED] * 2 + [
            _CELL_STATUS.NON_MALIGNANT] * 4
        assert (adata.obs[_CONSTANTS.MALIGNANT_ANNOTATION] == expected_annotation).all()

    @pytest.mark.parametrize("celltype,expected", [("evil", _CELL_STATUS.MALIGNANT),
                                                   ("unknown", _CELL_STATUS.UNDECIDED),
                                                   ("good", _CELL_STATUS.NON_MALIGNANT)])
    def test_annotate_malignant(self, celltype, expected):
        malignant_celltypes = ["evil"]
        undecided_celltypes = ["unknown"]

        assert _annotate_malignant(celltype, malignant_celltypes,
                                   undecided_celltypes) == expected
...
class TestLoadAdata:
    ...
    def test_load_from_file(self, adatas, tmpdir, subset):
        ...
        assert all([np.array_equal(adata.X, bdata.X) for adata, bdata in
                    zip(bdatas, adatas)])
...
class TestAnnotateReferenceGroups:
    @pytest.mark.parametrize("n_cells,valid_ref_groups,size_ref_groups", [
        ((0, 3, 3, 4), ["reference_group_1", "reference_group_2"], (0, 3, 3, 4)),
        ((2, 2, 2, 4), ["reference_group_0"], (6, 0, 0, 4))
    ])
    def test_annotate_reference_groups(self, adata, n_cells, valid_ref_groups,
                                           size_ref_groups):
        reference_groups = [("Reference_0",), ("Reference_1",), ("Reference_2",)]

        celltypes = ["Reference_0", "Reference_1", "Reference_2", "Non-Reference"]

        references = [f"{_REFERENCE.REFERENCE_PREFIX}_{i}" for i in range(3)] + [
            _REFERENCE.NON_REFERENCE]

        adata.obs["celltype"] = [celltype for n_cell, celltype in
                                 zip(n_cells, celltypes) for _ in range(n_cell)]

        expexted_refs = [reference for size_ref_group, reference in
                         zip(size_ref_groups, references) for _ in
                         range(size_ref_group)]

        assert get_reference_groups(adata, celltype_column="celltype",
                                    reference_groups=reference_groups,
                                    min_reference_cells=3,
                                    min_reference_groups=2) == valid_ref_groups
        assert (adata.obs[_CONSTANTS.REFERENCE_KEY] == expexted_refs).all()

    @pytest.mark.parametrize("celltype,expected",
                             [("Reference_tuple",
                               f"{_REFERENCE.REFERENCE_PREFIX}_0"),
                              ("Reference_str",
                               f"{_REFERENCE.REFERENCE_PREFIX}_1"),
                              ("Non_reference",
                               _REFERENCE.NON_REFERENCE)])
    def test_annotate_reference(self, celltype, expected):
        reference_groups = [("Reference_tuple",), "Reference_str"]
        assert _annotate_reference(celltype,
                                   reference_groups=reference_groups) == expected
