...
def adata():
    ...
    adata.obs[_CONSTANTS.MALIGNANT] = [_CELL_STATUS.NON_MALIGNANT] * 50 + [
        _CELL_STATUS.MALIGNANT] * 150
...
class TestSubclonalCluster:
    def test_subclonal_multiple_subclones(self, adata):
        ...
        assert np.equal(adata.obs["subclonal"][50:100],
                        adata.obs["subclonal"][50]).all()
        assert np.equal(adata.obs["subclonal"][100:150],
                        adata.obs["subclonal"][100]).all()
        assert np.equal(adata.obs["subclonal"][150:200],
                        adata.obs["subclonal"][150]).all()